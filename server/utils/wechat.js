...
async function sendTaskBidNotify({ openid, taskName, bidder, price, remark, taskId }) {
  ...
  const payload = {
    ...
    data: {
      thing1:  { value: trunc(taskName, 20) },
      thing2:  { value: trunc(bidder, 20) },
      amount3: { value: fmtAmount(price) },
      thing4:  { value: trunc(remark || '无', 20) },
    },
  };
 ...
 */
async function sendOrderStatusNotify({ openid, orderNo, title, status, time, taskId }) {
  ...
  const payload = {
    ...
    data: {
      ...
      thing2:            { value: trunc(title, 20) },
      phrase3:           { value: trunc(status, 15) },  // e.g. “进行中”
      time9:             { value: time || fmtTime(new Date()) },
    },
  };
 ...
 */
async function sendTaskAssignedToEmployee({ openid, serviceType, pickupAddr, deliveryAddr, fee, assignTime, taskId }) {
 ...
 */
async function sendTaskCompletedToEmployer({ openid, orderNo, amount, finishedAt, taskType = '跑腿', statusText = '雇员已完成任务，请前往确认完成' }) {
...
async function sendPayoutArrivedToEmployee({ openid, orderNo, amount, finishedAt, taskType = '跑腿', statusText = '任务已完成，报酬已入账' }) {
 ...
 */
async function sendClassReminder({ openid, courseName, address, teacher, timeText, tip = '请提前出发，别迟到哦～', page = 'pages/timetable/timetable' }) {
  ...
  const payload = {
    ...
    page,
    data: {
      thing6:  { value: trunc(courseName, 20) },
      thing10: { value: trunc(address || '无', 20) },
      thing13: { value: trunc(teacher || '老师', 20) },
      thing19: { value: trunc(timeText, 20) }, // “10:00-11:30”
      thing9:  { value: trunc(tip, 20) },
    },
  };
