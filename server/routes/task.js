...
router.post("/bid", async (req, res) => {
  const { task_id, user_id, price, advantage, can_finish_time } = req.body;
  ...
  try {
    ...
    res.json({ success: true, message: "投标成功，等待雇主选择" });
  } catch (err) {
...
router.post("/assign", async (req, res) => {
  const { task_id, employee_id, employer_id } = req.body;
  ...
  try {
    ...
    if (task.status !== 0) {
      return res.status(400).json({ success: false, message: "任务已被接单，无法修改" });
    }
    ...
    if (bids.length === 0) {
      return res.status(400).json({ success: false, message: "该用户未留言，无法指派" });
    }
    ...
    res.json({ success: true, message: "接单人指派成功" });
  } catch (err) {
