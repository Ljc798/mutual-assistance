...
router.post('/prepay-task', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const result = await c.curl('POST', '/v3/alipay/trade/pay', { body: { notify_url: notifyUrl, out_trade_no: outTradeNo, total_amount: totalAmount, subject } })
    return res.json({ success: true, out_trade_no: outTradeNo, total_amount: totalAmount, responseHttpStatus: result.responseHttpStatus, traceId: result.traceId, data: result.data })
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message })
  }
})
