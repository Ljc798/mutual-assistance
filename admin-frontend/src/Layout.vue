  ...
  <n-layout style="min-height: 100vh;">
    ...
    <n-layout-header class="layout-header" bordered>
      ...
      <div class="nav-buttons desktop-only">
        <n-space justify="space-between" size="large" style="width: 600px;">
          ...
          <n-button text :type="activeKey === '/pay' ? 'primary' : 'default'" @click="navigate('/pay')">
            <template #icon><n-icon>
                <SwapHorizontalOutline />
              </n-icon></template>
            转账管理
          </n-button>
          <n-button text :type="activeKey === '/review' ? 'primary' : 'default'" @click="navigate('/review')">
            <template #icon><n-icon>
                <DocumentTextOutline />
              </n-icon></template>
            反馈中心
    ...
    <n-drawer v-model:show="drawerVisible" placement="left" :width="200" resizable>
      <n-drawer-content title="导航菜单">
        ...
        <n-button block quaternary @click="navigate('/pay')">转账管理</n-button>
      </n-drawer-content>
    </n-drawer>
